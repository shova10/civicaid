...
function Legend() {
  return (
    <div className="absolute bottom-8 left-4 z-1000 bg-white/90 backdrop-blur-sm
      ...
      {LEGEND_ITEMS.map((item) => (
        <div key={item.priority} className="flex items-center gap-2">
          <span className="inline-block rounded-full border-2 shrink-0"
            style={{ width: 12, height: 12, backgroundColor: item.color + '55', borderColor: item.color }} />
...
function StatsBar({ issues }) {
  ...
  return (
    <div className="flex items-center gap-4 flex-wrap">
      <span className="text-sm font-semibold text-slate-700">{issues.length} issues plotted</span>
      <div className="flex items-center gap-2 flex-wrap">
        {LEGEND_ITEMS.map((item) =>
          counts[item.priority] ? (
            <span key={item.priority} className="text-xs font-semibold px-2 py-0.5 rounded-full"
...
export default function IssueMap() {
  ...
  const { filters, setCategory, setPriority, setStatus, filtered, reset, activeCount } =
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        // Heatmap empty — fall back to complaints with approximate Kathmandu coords
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Issue Map</h1>
            <p className="text-sm text-slate-400 mt-0.5">Live civic issues across the Kathmandu Valley</p>
          </div>
          {!loading && !error && <StatsBar issues={issues} />}
        </div>

        {!loading && !error && (
          <MapFilters filters={filters} setCategory={setCategory} setPriority={setPriority}
            setStatus={setStatus} reset={reset} activeCount={activeCount}
            total={issues.length} showing={filtered.length} />
        )}
        ...
        {error ? (
          <div className="flex flex-col items-center justify-center h-96 rounded-2xl bg-white border border-slate-200">
            ...
            <p className="text-slate-700 font-semibold mb-1">Failed to load map data</p>
            ...
            {loading && (
              <div className="absolute inset-0 z-1001 flex items-center justify-center
                ...
                <div className="flex items-center gap-3 bg-white rounded-xl shadow-md border border-slate-200 px-5 py-3">
                  ...
                  <span className="text-sm font-medium text-slate-600">Loading issues…</span>
                </div>
              </div>
            )}
            ...
            <NepalMap className="h-140">
              <MarkerClusterGroup>
                {filtered
                  ...
                  .map((issue) => {
                    const style = PRIORITY_STYLE[issue.priority] ?? PRIORITY_STYLE.none
                    return (
                      <CircleMarker key={issue.id} center={[issue.lat, issue.lng]} radius={style.radius}
                        pathOptions={{ color: style.color, fillColor: style.fillColor, fillOpacity: 0.45, weight: 2 }}>
                        <Popup>
                          <div className="p-1 min-w-50">
                            <p className="text-sm font-semibold text-slate-800 leading-snug mb-2">{issue.title}</p>
                            <div className="flex items-center gap-1.5 mb-3">
                              <StatusBadge status={issue.status} size="sm" />
                              <PriorityBadge priority={issue.priority} size="sm" />
                            </div>
                            <p className="text-xs text-slate-400 mb-3">{issue.category}</p>
